...
/**
 * One-time menu creator — visit /wp-admin/?mci_create_menus=1 to run.
 * Remove this block after menus are created.
 */
function mci_create_lang_menus() {
	if ( ! current_user_can( 'manage_options' ) || empty( $_GET['mci_create_menus'] ) ) {
		return;
	}

	$home = untrailingslashit( home_url() );

	// [ slug, Russian label, Greek label ]
	$primary_items = array(
	...
	$footer_items = array_merge( $primary_items, array(
	...
	foreach ( $menus as list( $slug, $name, $location, $items, $lang_idx ) ) {
		...
		if ( is_wp_error( $menu_id ) ) {
			$results[] = "FAIL: {$name} — " . $menu_id->get_error_message();
			continue;
		}
		...
		foreach ( $items as $item ) {
			...
			wp_update_nav_menu_item( $menu_id, 0, array(
				'menu-item-title'    => $item[ $lang_idx ],
				'menu-item-url'      => $home . $item[0],
		...
		$results[] = "OK: {$name} ({$pos} items) → {$location}";
	}

	wp_die(
		'<h2>Menu Creation Results</h2><pre>' . esc_html( implode( "\n", $results ) ) . '</pre><p>Done! You can now remove the <code>mci_create_lang_menus</code> block from functions.php.</p>',
		'Menus Created',
		array( 'response' => 200 )
	);
}
add_action( 'admin_init', 'mci_create_lang_menus' );
